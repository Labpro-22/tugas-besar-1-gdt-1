...
CommandResult CommandProcessor::process(const std::string &command, Player *player)
{
    ...
    try
    {
        ...
        if (cmd == "TEBUS")
        {
            if (tokens.size() < 2)
            {
                gui->showMessage("Format: TEBUS <kode>");
                return CommandResult::INVALID;
            }
        ...
        if (cmd == "BANGUN")
        {
            if (tokens.size() < 2)
            {
                gui->showMessage("Format: BANGUN <kode>");
                return CommandResult::INVALID;
            }
        ...
        if (cmd == "BUILD")
        {
            ...
            if (tokens.size() < 2)
            {
                gui->showMessage("Format: BUILD <kode>");
                return CommandResult::INVALID;
            }
            return handleBuild(player, tokens[1]);
        }
        if (cmd == "GUNAKAN_KEMAMPUAN")
        {
            if (tokens.size() < 2)
            {
                gui->showMessage("Format: GUNAKAN_KEMAMPUAN <nomor_kartu>");
                return CommandResult::INVALID;
            }
            return handleUseSkill(player, std::stoi(tokens[1]) - 1);
        }
        if (cmd == "CETAK_KARTU")
            return handleShowCards(player);
        if (cmd == "BUANG_KARTU")
        {
            if (tokens.size() < 2)
            {
                gui->showMessage("Format: BUANG_KARTU <nomor_kartu>");
                return CommandResult::INVALID;
            }
        ...
        if (cmd == "SIMPAN")
        {
            if (tokens.size() < 2)
            {
                gui->showMessage("Format: SIMPAN <file>");
                return CommandResult::INVALID;
            }
        ...
        if (cmd == "MUAT")
        {
            if (tokens.size() < 2)
            {
                gui->showMessage("Format: MUAT <nama_save>");
                return CommandResult::INVALID;
            }
        ...
        if (cmd == "FESTIVAL")
        {
            if (tokens.size() < 2)
            {
                gui->showMessage("Format: FESTIVAL <kode>");
                return CommandResult::INVALID;
            }
...
CommandResult CommandProcessor::handleBuild(Player *player, const std::string &code)
{
    ...
    auto *sp = (property != nullptr && property->isStreet())
                   ? static_cast<StreetProperty *>(property)
                   : nullptr;
...
CommandResult CommandProcessor::handleDropCard(Player *player, int index)
{
    ...
    if (game->getSkillDeck() != nullptr)
        game->getSkillDeck()->discard(card);
    if (engine->logger != nullptr)
    {
        engine->logger->log(game->getCurrentTurn(), player->getUsername(),
...
bool CommandProcessor::applyMoveCard(Player *player, MoveCard *card)
{
    ...
    {
        engine->logger->log(game->getCurrentTurn(), player->getUsername(),
                            "KARTU", "Pakai MoveCard maju " + std::to_string(steps) + " petak -> " + tileName);
    }
    if (landed)
        engine->handleTileLandingPublic(player, landed);
    return true;
}
...
bool CommandProcessor::applyTeleportCard(Player *player, TeleportCard * /*card*/)
{
    ...
    while (true)
    {
        ...
        {
            engine->logger->log(game->getCurrentTurn(), player->getUsername(),
                                "KARTU", "Pakai TeleportCard -> pindah ke " + target->getName() + " (" + code + ")");
        }
...
bool CommandProcessor::applyDiscountCard(Player *player, DiscountCard *card)
{
    ...
    {
        engine->logger->log(game->getCurrentTurn(), player->getUsername(),
                            "KARTU", "Pakai DiscountCard diskon " + std::to_string(card->getDiscount()) + "%");
    }
    return true;
}
...
bool CommandProcessor::applyLassoCard(Player *player, LassoCard * /*card*/)
{
    ...
    {
        if (other == player || other->isJailed())
            continue;
    ...
    {
        engine->logger->log(game->getCurrentTurn(), player->getUsername(),
                            "KARTU", "Pakai LassoCard -> tarik " + target->getUsername() + " ke petak " + std::to_string(myPos));
    }
    return true;
}
...
bool CommandProcessor::applyDemolitionCard(Player *player, DemolitionCard * /*card*/)
{
    ...
    if (board != nullptr)
    {
        for (Tile *tile : board->getAllTiles())
        {
            ...
            if (prop == nullptr || !prop->isStreet() || prop->isMortgaged())
                continue;

            Player *owner = prop->getOwner();
            if (owner == nullptr || owner == player || owner->getStatus() == PlayerStatus::BANKRUPT)
                continue;
    ...
    std::string before = sp->getBuildingState() == BuildingState::HOTEL ? "Hotel" : std::to_string(static_cast<int>(sp->getBuildingState())) + " rumah";
    sp->demolishOneLevel();
    ...
    {
        engine->logger->log(game->getCurrentTurn(), player->getUsername(),
                            "KARTU", "Pakai DemolitionCard -> hancur 1 bangunan di " + sp->getName() + " (" + sp->getCode() + ") milik " + targets[choice].first->getUsername());
    }
    return true;
}
