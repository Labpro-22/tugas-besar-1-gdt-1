...
CameraMovement::CameraMovement(View3DCamera& camera, const float fps, bool interuptable, function<void()> animationFunc, function<void()> animationEndFunc) :
    camera(camera), ViewAnimation(fps, interuptable, animationFunc, animationEndFunc) {
        playing = true;
}
...
void CameraMovement::setMoveToCameraAnimation(View3DCamera& destCamera, const float duration) {
    ...
    animationFunc = [destCamera, duration, startingPos, startingTargetPos, startingUp, this](){
        if (frameProgress/fps >= duration) {
